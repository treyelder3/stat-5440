...
bike_model <- stan_glm(rides ~ temp_feel, data = bikes,
                       ...
                       prior = normal(100, 40), 
                       prior_aux = rstanarm::exponential(rate = 0.0008),
...
# BRMS Capital Bike share Example
library(brms)

# ensure clean salte 
data(bikes)

# center key predictor to make intercept more interpretable
bikes$temp_feel <- bikes$temp_feel - mean(bikes$temp_feel)

# fit the model in brms
bike_model_brms <- brm(
  formula = rides ~ temp_feel, #specify response and predictor(s)
  data    = bikes, #data frame
  family  = gaussian(), #specify response distribution
  prior   = c(
    prior(normal(5000, 1000), class = "Intercept"), #prior for intercept
    prior(normal(100, 40), class = "b", coef = "temp_feel"), #prior for slope
    # can specify one prior for all slopes with: prior(normal(0, 40), class = "b"),
    prior(exponential(0.0008), class = "sigma") #prior for error sd
  ),
  chains = 4, # of MCMC chains
  cores = 4, # of cores to use in parallel
  iter   = 10000, # number of iterations per chain
  seed   = 84735 # set seed for reproducibility,
)

summary(bike_model_brms) #summary of posterior

# Family: gaussian 
# Links: mu = identity; sigma = identity 
# Formula: rides ~ temp_feel 
# Data: bikes (Number of observations: 500) 
# Draws: 4 chains, each with iter = 10000; warmup = 5000; thin = 1;
# total post-warmup draws = 20000

# Regression Coefficients:
#   Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
# Intercept  3487.18     58.20  3373.74  3603.28 1.00    19119    14544
# temp_feel    82.15      5.12    72.19    92.28 1.00    21003    15274

# Further Distributional Parameters:
#   Estimate Est.Error l-95% CI u-95% CI Rhat Bulk_ESS Tail_ESS
# sigma  1282.99     40.78  1206.94  1366.78 1.00    21086    15651

# Draws were sampled using sampling(NUTS). For each parameter, Bulk_ESS
# and Tail_ESS are effective sample size measures, and Rhat is the potential
# scale reduction factor on split chains (at convergence, Rhat = 1).

plot(bike_model_brms)  # plots histograms of coefficient posteriors and trace plots

pp_check(bike_model_brms, nsamples = 100) 
# draws nsamples of size n from posterior predictive distribution and overlays on observed data

ce <- conditional_effects(bike_model_brms, effects = "temp_feel")
plot(ce, points = TRUE) # plot conditional effects of temp_feel on rides with original data

check_hmc_diagnostics(bike_model_brms$fit)
# check for divergent transitions, max tree depth issues, and low E-BFMI
